"""Accs API (Agent OS V0.4 slice, "give Acc a name and a face").
...
surface needs: an Acc belonging to a *different* workspace must 404.
"""
...
@pytest.mark.asyncio
async def test_create_acc_then_appears_in_list(
    workspace_http_client, auth_headers, seeded_project
):
    create = await workspace_http_client.post(
        "/accs",
        json={"name": "Research Acc", "goal": "Track competitor pricing", "project_id": seeded_project},
        headers=auth_headers,
    )
    assert create.status_code == 200
    body = create.json()
    assert body["name"] == "Research Acc"
    assert body["goal"] == "Track competitor pricing"
    assert body["status"] == "active"
    assert body["project_id"] == seeded_project

    listing = await workspace_http_client.get("/accs", headers=auth_headers)
    assert listing.status_code == 200
    assert [a["id"] for a in listing.json()["accs"]] == [body["id"]]


@pytest.mark.asyncio
async def test_acc_detail_reports_work_from_its_project_jobs(
    workspace_http_client, auth_headers, seeded_project
):
    create = await workspace_http_client.post(
        "/accs",
        json={"name": "Research Acc", "project_id": seeded_project},
        headers=auth_headers,
    )
    acc_id = create.json()["id"]

    detail = await workspace_http_client.get(f"/accs/{acc_id}", headers=auth_headers)
...
@pytest.mark.asyncio
async def test_patch_acc_renames(workspace_http_client, auth_headers, seeded_project):
    create = await workspace_http_client.post(
        "/accs",
        json={"name": "Research Acc", "project_id": seeded_project},
        headers=auth_headers,
    ...
    patched = await workspace_http_client.patch(
        f"/accs/{acc_id}", json={"name": "Renamed Acc"}, headers=auth_headers
    )
    assert patched.status_code == 200
    assert patched.json()["name"] == "Renamed Acc"


@pytest.mark.asyncio
async def test_acc_from_another_workspace_is_not_found(
    ...
    response = await workspace_http_client.get(
        f"/accs/{acc_id}", headers={"Authorization": f"Bearer {other_token}"}
    )
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_create_acc_for_unowned_project_is_not_found(
    workspace_http_client, keypair, auth_headers, seeded_project
):
    other_token = _make_token(keypair, sub=str(uuid.uuid4()), email="other@example.com")
